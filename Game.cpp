...
void Game::PlayRound()
{
	...
	while (true)
	{
		...
		if (board.CanActivateExplosion() && !explosionTriggered)
		{
			Player& currentPlayer = (currentPlayerId == 1) ? player2 : player1;
			...
			{
				board.ActivateExplosion(currentPlayer);
		...
		if (cardValue == -1)
		{
			...
			if (board.PlaceIllusion(row, col, currentPlayerId, illusionCardValue))
			{
				...
				std::cout << "Illusion placed succesfully.\n";
				currentPlayerId = (currentPlayerId == 1) ? 2 : 1;
				board.Display();
			}
			else std::cout << "The selected position is not valid. Try again.\n";
		}
		else
		{
			...
			{
				std::cout << "Your card has been lost due to the illsion.\n";
			...
			else if (result == PlaceCardResult::Success)
			{
				if (currentPlayer.PlayCard(cardValue))
				{
					...
					if (board.IsFull())
					{
						...
						std::cout << "Player 1's total card sum: " << player1Sum << "\m";
						std::cout << "Player 2's total card sum: " << player2Sum << "\m";
